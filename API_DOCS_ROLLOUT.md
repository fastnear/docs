...
This file tracks the multi-repo API docs rollout for the current `docs-pipeline` architecture phase.

## Locked Decisions

- Specs are owned in the service repos and vendored into `docs-pipeline`.
- Redocly namespaces are split per service:
  ...
  - `apis/kv-fastdata`
  - `apis/neardata`
- `builder-docs` uses one page per operation with iframe embeds from this repo.
...
| Repo | Inventory complete | Spec source chosen | Spec authored/generated | Synced into `docs-pipeline` | Redocly lint green | Builder-docs pages added | Manual smoke test done |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
- `npm run lint` is green in `docs-pipeline`.
- `REDOCLY_LOCAL_PLAN=enterprise npm run build` is green in `docs-pipeline`; the local static build now completes end-to-end.
...
- `npm run build` in `docs-pipeline` now prefers `PLAN_GATES` from the shell environment or `.env.redocly.local`, with a local-only fallback via `REDOCLY_LOCAL_PLAN=enterprise|pro` for developer validation.
