...
function getOutputModelPath(pageModelId) {
  return path.resolve(ROOT, `standalone/generated/${pageModelId}-page-model.json`);
}
...
const PAGE_SPECS = [...loadRpcPageSpecs(), ...loadCustomPageSpecsFromEnhancements()].map(
  (pageSpec) => ({
    ...pageSpec,
    outputModel: pageSpec.outputModel || getOutputModelPath(pageSpec.pageModelId),
  })
);
const SOURCE_SPEC = PAGE_SPECS[0]?.sourceSpec;
const SOURCE_SPECS = PAGE_SPECS.map(({ sourceSpec }) => sourceSpec);
const OUTPUT_MODEL = PAGE_SPECS[0]?.outputModel;
...
function buildStandalonePageModel(pageSpec) {
  ...
  return {
    ...
    outputModel: toPosixRelative(pageSpec.outputModel),
...
function buildStandaloneViewAccountModel() {
  return buildStandalonePageModel(PAGE_SPECS[0]);
}

function writeStandalonePageModels() {
  const models = PAGE_SPECS.map((pageSpec) => {
    const model = buildStandalonePageModel(pageSpec);
    fs.mkdirSync(path.dirname(pageSpec.outputModel), { recursive: true });
    fs.writeFileSync(pageSpec.outputModel, `${JSON.stringify(model, null, 2)}\n`, "utf8");
    return model;
  });
...
function writeGeneratedFastnearPageModelsModule(models) {
  const entries = models.map((model) => ({
    exportName: `${toIdentifier(model.pageModelId)}PageModel`,
    importPath: `../${model.outputModel}`,
  }));
...
function writeGeneratedPageModelArtifacts() {
  const models = writeStandalonePageModels();
  writeGeneratedFastnearPageModelsModule(models);
  writeGeneratedFastnearPageModelsJson(models);
  return models;
}

function writeStandaloneViewAccountModel() {
  const [model] = writeGeneratedPageModelArtifacts();
  return model;
}
...
if (require.main === module) {
  ...
  for (const model of models) {
    console.log(`Generated standalone model: ${model.sourceSpec} -> ${model.outputModel}`);
  }
}
...
module.exports = {
  ...
  SOURCE_SPEC,
  SOURCE_SPECS,
  buildStandaloneViewAccountModel,
  buildStandalonePageModel,
  writeGeneratedPageModelArtifacts,
  writeStandalonePageModels,
  writeStandaloneViewAccountModel,
};
